...
function isValidSunoMusicRequestArgs(args) {
  ...
  if (args.mv !== void 0 && !["chirp-v3-0", "chirp-v3-5", "chirp-v4"].includes(args.mv)) return false;
  if (args.make_instrumental !== void 0 && typeof args.make_instrumental !== "boolean") return false;
  const hasTaskId = args.task_id !== void 0 && typeof args.task_id === "string" && args.task_id.trim() !== "";
  const hasContinueAt = args.continue_at !== void 0 && typeof args.continue_at === "number" && args.continue_at >= 0;
  const hasContinueClipId = args.continue_clip_id !== void 0 && typeof args.continue_clip_id === "string" && args.continue_clip_id.trim() !== "";
...
var SUNO_API_KEY = process.env.SunoKey;
if (!SUNO_API_KEY) {
...
var SUNO_API_CONFIG = {
  ...
  ENDPOINTS: {
    ...
    FETCH_TASK: "/suno/fetch/"
    // Append task_id
  },
  POLLING_INTERVAL_MS: 5e3,
  // 5 seconds
  MAX_POLLING_ATTEMPTS: 60
  // 5 minutes max polling (5s * 60 = 300s)
};
var SunoMcpServer = class {
  server;
  sunoApiAxiosInstance;
  constructor() {
    ...
    this.sunoApiAxiosInstance = axios_default.create({
  ...
  setupErrorHandling() {
    this.server.onerror = (error) => {
      console.error("[MCP Error] \u03A3(\xB0\u0414\xB0lll) Waaah! An error occurred nya:", error instanceof Error ? error.message : error);
    };
  ...
  async handleGenerateMusicTool(args) {
    ...
    if (args.task_id && args.continue_at !== void 0 && args.continue_clip_id) {
    ...
    try {
      const submitResponse = await this.sunoApiAxiosInstance.post(
      ...
      while (attempts < SUNO_API_CONFIG.MAX_POLLING_ATTEMPTS) {
        ...
        const fetchResponse = await this.sunoApiAxiosInstance.get(
...
var server = new SunoMcpServer();
server.run().catch((error) => {
  console.error("\u03A3(\xB0\u0414\xB0lll) Failed to start Suno MCP server nya:", error);
  process.exit(1);
});
